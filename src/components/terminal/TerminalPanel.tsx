...
/** Payload shape shared by every PTY stream event (docker + host shells). */
...
/** How the panel talks to one kind of shell (docker exec, host SSH, local
 * PTY). The panel treats the target as fixed — parents remount it (key) to
 * switch targets. */
export interface TerminalBackend {
  /** Tauri event carrying PtyOutput for this backend. */
  event: string;
  open: (cols: number, rows: number) => Promise<number>;
  write: (session: number, data: number[]) => Promise<unknown>;
  resize: (session: number, cols: number, rows: number) => Promise<unknown>;
  close: (session: number) => unknown;
  /** Command lines from previous sessions, oldest first. */
  fetchHistory?: () => Promise<string[]>;
}

/** Right slide-over hosting a real PTY. Deliberately NOT the shared Drawer:
 * Esc must reach the shell (vim, less), so closing is X button / backdrop
 * only. */
export function TerminalPanel({
  ...
  // The mount-once effect reads the backend through a ref so a parent
  // re-render (new object identity) doesn't tear the session down.
  ...
  const pasteCommand = (cmd: string) => {
    ...
    backendRef.current.write(sessionRef.current, data).catch(() => {});
  ...
  useEffect(() => {
    ...
    const unlistenP = listen<PtyOutput>(b.event, (e) => {
    ...
    const dataSub = term.onData((s) => {
      if (session !== null) {
        b.write(session, Array.from(encoder.encode(s))).catch(() => {});
      }
    });
    ...
    const ro = new ResizeObserver(() => {
      fit.fit();
      if (session !== null) {
        b.resize(session, term.cols, term.rows).catch(() => {});
      }
    });
    ro.observe(host);
    ...
    return () => {
      ...
      if (session !== null) b.close(session);
        ...
        {history.length > 0 && (
          <div className="flex items-center gap-1.5 overflow-x-auto border-b border-border px-4 py-2">
            <span
              ...
              title="Commands from previous shells here — click to type one into the prompt"
            >
              <History size={11} />
              History
            </span>
